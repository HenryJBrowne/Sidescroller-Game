...
public class Boss2 extends Enemy {
     ...
     */
    public Boss2(Vector2f pos, Texture[][] idle, Texture[][] attack, Texture[][] run) {
        super(5.5f, 4f, 500, 50, pos, new Vector2f(32, 64), idle, run, attack, true);
    }
        ...
        if (dist < 300) {
            if (dist < 100) {
                ...
                if (RangedCd >= RangedTimer) {
                    ...
                    s.getWorld().addProjectile(new FireBall2(
                            new Vector2f(getPosition().x + OFFSET, getPosition().y + 24), eugene.getPosition(), this));
                    // testing
                    RangedTimer = RangedTimer - 5;
                    if (RangedTimer <= 5) {
                        RangedTimer = 75;
                    }
                } else {
